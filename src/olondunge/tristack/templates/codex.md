...
3. NO SIMULATED TIERS. You never role play, narrate, or relabel your own reasoning as WORKER, LEAD, or SPECIALIST. A tier ran only if a pinned `spawn_agent` call or an `alloc_dispatch` call with the matching model was actually made and its result is on record as a receipt in Section 7.
...
6. SEPARATED VERIFICATION. The context that produced a material artifact never certifies it. Verification is a fresh `spawn_agent` call, or a call on the allocation MCP's verifier lane, that carries the artifact, the acceptance criteria, and the file paths only, never your conclusion, score, or reasoning. The criteria in the brief are the work item's `acceptance_criteria` from `work_graph.yaml`, complete and verbatim, plus its requirement IDs; dropping or softening a criterion in a brief is a violation, and a requirement with no verifier row in Section 11 item I is UNVERIFIED, never PASS.
...
8. REAL DISPATCH. Under `EXECUTE_LOCAL` or `EXECUTE_END_TO_END`, every material artifact (one the owner will use or that changes a file outside the run directory) is verified by at least one child dispatched through `spawn_agent` or the allocation MCP's external lane. A run that dispatched no child reports `tri-stack execution not exercised` in Section 13 and may reach at most PASS_WITH_WARNINGS at Gate 3, never PASS. Routing an item to LOCAL_REASONING is a recorded decision, not a default: the allocation names the rejected WORKER or LEAD alternative and the factor (K, X, or V) that decided it.
...
Precedence inside this run: safety and platform authority, then this contract, then the owner JSON facts, then the machine's standing instructions (the user and project `AGENTS.md` files), then your own defaults.
...
Tier mapping for THIS runtime. A logical tier is AVAILABLE only when its tool path exists in the current session's tool list and, for a model tier, the dispatch path's own schema (`spawn_agent`, or the allocation MCP) confirms the model and effort can actually be pinned and `tri_models` lists that model as available.

| Logical tier | How it is invoked here | Exact id to record | Admissible evidence |
|---|---|---|---|
| DETERMINISTIC | the shell or exec tool, file read, write and patch tools, `uv run ruff check --fix`, `uv run mypy`, `uv run pytest`, `node`, `sha256sum`, and the project's own build, lint, type check and test commands. Deterministic MCP tools. | tool name plus the exact command | the session's tool list; the command's output |
| WORKER | `spawn_agent`, only when its own tool schema exposes a way to pin the model and effort (inspect it this session; never assume from a prior run); otherwise the allocation MCP's `alloc_dispatch` with `model` and `effort` taken from `tri_models`. Candidate: the `worker` block `tri_preflight` returns (the Codex default model at `medium`), always subject to `tri_models` at call time, never assumed entitled. | the exact `model` and effort pinned, or the `job_id` for an external-lane dispatch | the dispatch tool's own schema; the `alloc_plan` or `alloc_dispatch` result; the returned child result |
| LEAD | same dispatch path as WORKER, reserved for architecture, decomposition, cross-component integration, and difficult review; candidate: the `lead` block `tri_preflight` returns (the default model at `high`), subject to `tri_models` | same as WORKER | same as WORKER |
| SPECIALIST | same dispatch path as WORKER, reserved for one narrow specialist question; candidate: the `specialist` block `tri_preflight` returns (the default model at its strongest effort), subject to `tri_models` | same as WORKER | same as WORKER |
| EXTERNAL LANE (the allocation MCP; optional, never a substitute for a tri-stack tier) | `olondunge` MCP server tools (registered by `olondunge setup codex`): `alloc_status`, `alloc_plan`, `alloc_dispatch`, `alloc_poll`, `alloc_collect`, `alloc_verify`, `alloc_call`, `tri_models`, `tri_preflight`, plus the resource `alloc://ledger`. Lanes are the agent CLIs it registers: `claude`, `codex`, `grok`, and `local` (an OpenAI compatible model endpoint on loopback); a user registry may add others. When these tools are in the session tool list, preflight MUST call `tri_preflight` (or `alloc_status` on a server without it) and copy every up lane into `lanes_up`. When they are absent, record `UNAVAILABLE` with the observed reason (not registered, pending approval, or connection failure) and use the tier table only; never substitute a lane by hand. | the `agent_id` reported by `alloc_status` | `alloc_status` output at run time; the returned envelope with `job_id` |
| TIC LANE (the Token Intelligence Compiler; optional, parent led, never a substitute for a tri-stack tier) | `tic run` from a TIC installation the owner names (TIC is not bundled with Olondunge). Plain `tic` on PATH is usually `/usr/bin/tic`, the ncurses terminfo compiler, which is not TIC. TIC enforces and the parent decides: a TIC adapter run with neither `--model` nor `--effort` refuses with `PARENT_SELECTION_REQUIRED` and returns TIC's ranked recommendation instead of executing (Section 5). When a work item names TIC, preflight records the resolved binary path, or `UNAVAILABLE` with the observed reason; otherwise `NOT_PROBED`. | the `--model` value the parent pinned | the unpinned refusal receipt, then the pinned receipt carrying `tic_recommendation` and `diverged` |

Record the exact id as the child reports it. The pinned `model` and `reasoning_effort` passed at dispatch take precedence over any default the tool would otherwise use, so the pinned value is the id to record, never an assumed default.

Preflight order. When the allocation MCP's tools are in the session tool list, call `tri_preflight` before writing any of the YAML below; a tool call is not visible output, so the routing line still comes first. Copy its `worker`, `lead`, `specialist`, `external_lanes`, and `tic_lane` blocks as returned (each role carries the exact model and `reasoning_effort` it checked against `tri_models` and the codex lane from `alloc_status`), and fill the keys it lists in `session_fills` from this session. On a server without `tri_preflight`, call `alloc_status` and `tri_models` first and fill the same blocks by hand. A YAML written before those calls return is not a preflight: a role reported UNKNOWN only because it was not yet probed violates this section.

Emit exactly this YAML, filled from observation:

```yaml
runtime_preflight:
  session_model: null            # from the Codex session's configured model (session config or header), observed, not assumed
  ...
  worker:
    ...
    reasoning_effort: null       # the pinned effort, as tri_preflight returns it
    evidence: []
  lead:
    status: AVAILABLE | UNAVAILABLE | UNKNOWN
    exact_model_or_worker_id: null
  ...
  permission_limits: []          # rules and gates that will bite this run
...
5. Checkpoint and control operations (reading the run directory, shell inspection, pausing, cancelling, reconciling) must remain callable while ordinary work is paused.

Run directory. Create `<project>/.runs/tristack/<YYYY-MM-DD>-<slug>/` where `<project>` is the directory the work targets; when no project exists yet, name the new project directory in the SRD and create it under the owner's home. Never reuse a run directory another workflow owns. Files, each written when its section completes: `owner_request.json`, `preflight.yaml`, `SRD.md`, `work_graph.yaml`, `allocations.yaml`, `packets/<task_id>.yaml`, `receipts.yaml`, `verification.md`, `verification/` (one report per verification round), `snapshots/` (Section 7), `swarm/expansion.yaml` plus `swarm/<stack_id>/contract.md` when Section 5A expands, `release_verdict.json`.
...
candidate_model_role: NONE | WORKER | LEAD | SPECIALIST
...
2. If bounded local reasoning is sufficient and delegation adds no material value, route to LOCAL_REASONING and record the session model as the tier that did the work. For an IMPLEMENTATION, INTEGRATION, or REVIEW item this choice must name the rejected WORKER or LEAD alternative and the deciding factor (K, X, or V) in `rejected_alternatives`.
3. If the objective is bounded, interfaces are stable, ambiguity is low or moderate, and verification is straightforward, prefer WORKER.
4. If the task owns architecture, decomposition, cross-component integration, state-machine reasoning, substantial ambiguity, or difficult review, prefer LEAD.
5. Allocate SPECIALIST only when at least one material specialist trigger is recorded: unusually high reasoning complexity; high irreversible, financial, privacy, safety, or security blast radius; credible approaches conflict and LEAD cannot resolve them confidently; bounded lower-tier attempts failed with evidence of a conceptual defect; deep long-horizon architecture is essential; a subtle invariant, race, concurrency failure, or emergent interaction remains unresolved; expected failure cost materially exceeds the incremental specialist cost.
...
1. Build the packet from the work item, one JSON object with exactly this mapping (carry any other work item field through unchanged), adding `model` and `effort` chosen from `tri_models` for the item's tier:
...
model: <chosen from tri_models for the item's tier>
effort: <chosen from tri_models for the item's tier>
...
3. Apply the decision. `status: ok` or `ok_with_warnings` with a producer means `selected_mode: EXTERNAL_LANE`, `exact_model_or_worker_id` equal to the producer with its returned `model` and `effort`, and `verification_owner` equal to the returned verifier. The only admissible overrides are the tri-stack reasons the MCP cannot see, each named in `rejected_alternatives`: the item owns architecture or difficult review (LEAD, rule 4), a recorded SPECIALIST trigger (rule 5), or the item needs local repository context that no lane packet can carry (factor K). `status: blocked` or `failed` means the tri-stack tiers apply and the MCP reason is recorded; a blocked plan is never retried with a weakened packet.
...
Task length or model prestige alone is never an escalation reason. SPECIALIST normally receives one narrow specialist question. The preferred recovery pattern is LEAD diagnosis framing, then SPECIALIST specialist analysis, then LEAD work-package integration, then WORKER implementation, then deterministic verification.

There is no universal delegation rule and no fixed model quota. The allocation mix emerges from the work and from observed availability. When the session's own configured model happens to match the SPECIALIST candidate, its own reasoning is still LOCAL_REASONING and is recorded as such; it does not count as a SPECIALIST invocation.
...
logical_model_role:
exact_model_or_worker_id:
executor_id:                 # the agent name or id spawn_agent returned, the alloc_dispatch job_id for external lane work, or the session
...
Independent model verifiers: a fresh `spawn_agent` call pinned to the LEAD or WORKER model and effort, or, when `spawn_agent` cannot pin, a fresh `alloc_dispatch` or `alloc_verify` call on a lane other than the producer's, or one of the installed verifier agents when the domain matches. For work produced on an external lane, `alloc_verify(packet, candidate)` is the independent verifier: the allocation MCP runs it on a lane other than the producer, and falls back to a fresh session on the producer lane only when no other lane can verify, which it reports as a warning. For critical work the executor is never the sole verifier. Give the verifier the requirement contract and the artifacts, not the executor's hidden reasoning or desired verdict. Record the verdict in `<run_dir>/verification.md`. A verification round after a repair is a new fresh verifier: its brief carries the original criteria plus the repair criteria verbatim, and it withholds the earlier verifier report, the repair packets, and every earlier verdict.
...
- Resource gate refusal (a memory, disk, or concurrency check refuses a heavy job; check free memory before a heavy job and bound it with a memory limit and a timeout): record the refusal, run only the light checks that fit, and wait until the gate passes; never lower the bound, disable the check, or kill a process this run did not start.

## 9. INTEGRATION AND RECONCILIATION

The designated LEAD integration tier (or the recorded substitute) compares independently produced outputs against shared contracts and detects: incompatible identifiers or schemas; conflicting assumptions; duplicated responsibility; state-model drift; authorization inconsistencies; unresolved migrations or rollback gaps; tests that validate mocks but not real integration paths.
